...
namespace _365_Portal.Controllers
{
    public class OrganizationController : ApiController
    {
                ...
                if (identity.Role == ConstantMessages.Roles.companyadmin || identity.Role == ConstantMessages.Roles.superadmin)
                {
                    if (ValidateUserDetails(jsonResult, out Message, out objUser, "create"))
                    {
                        ...
                        if (!string.IsNullOrEmpty(userProfilePicBase64))
                        {
                            ...
                            string fileName = objUser.UserID + "_" + Guid.NewGuid() + "." + Utility.GetFileExtension(userProfilePicBase64);
                            ...
                            {
                                objUser.ProfilePicFileID = dsProfilePic.Tables[0].Rows[0]["UniqueID"].ToString();
                            }
                        }
                        ...
                        if (!string.IsNullOrEmpty(companyLogoBase64))
                        {
                            ...
                            string fileName = objUser.UserID + "_" + Guid.NewGuid() + "." + Utility.GetFileExtension(companyLogoBase64);
                            ...
                            {
                                objUser.CompanyProfilePicFileID = dsCompanyLogo.Tables[0].Rows[0]["UniqueID"].ToString();
                            }
                        }
                        objUser.ThemeColor = (string)jsonResult.SelectToken("CompanyThemeColor");
                        objUser.ThemeColor2 = (string)jsonResult.SelectToken("CompanyThemeColor2");
                        objUser.ThemeColor3 = (string)jsonResult.SelectToken("CompanyThemeColor3");
                        objUser.ThemeColor4 = (string)jsonResult.SelectToken("CompanyThemeColor4");
        ...
        private bool ValidateUserDetails(JObject jsonResult, out string Message, out UserBO objUserVal, string flag)
        {
            ...
            {
                objUserVal.EmployeeCount = (string)jsonResult.SelectToken("EmployeeCount");
            }
            else
            {
