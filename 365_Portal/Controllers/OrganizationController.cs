    ...
    public class OrganizationController : ApiController
    {
        ...
        public IHttpActionResult CreateAdminUser(JObject jsonResult)
        {
            ...
            if (identity != null)
            {
                ...
                {
                    if (ValidateUserDetails(jsonResult, out Message, out objUser, "create", Convert.ToInt32(identity.UserID), identity.Role))
                    {
                        objUser.UserID = identity.UserID;
                        objUser.CompId = identity.CompId;
                        objUser.Role = identity.Role;
        ...
        private bool ValidateUserDetails(JObject jsonResult, out string Message, out UserBO objUserVal, string flag,int ChildUserID,string Role)
        {
            bool ValFlag = true;
            ...
            if (identity != null)
            {
                ...
                if (identity.Role == ConstantMessages.Roles.companyadmin || identity.Role == ConstantMessages.Roles.superadmin)
                {
                    ...
                    if (jsonResult.SelectToken("UserID") != null && jsonResult.SelectToken("UserID").ToString().Trim() != "")
                    {
                        ...
                        if (ValidateUserDetails(jsonResult, out Message, out objUser, "update", ChildUserID, identity.Role))
                        {
                            objUser.UserID = identity.UserID;
                            objUser.CompId = identity.CompId;
                            objUser.Role = identity.Role;
